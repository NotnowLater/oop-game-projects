...
public class BirdShootingEnemy extends Enemy{
    ...
    public BirdShootingEnemy(GameWindow gW, Vector2 worldPos, String spritePath, int element){
        super(gW, worldPos);
    ...
    public void shootPlayer(){
        for (int i = -1; i < 2; i++){
            Projectile p = new DarkBirdShootingProjectile(getgW(), new Vector2(getWorldPosition().getX() , getWorldPosition().getY()), "sprites/enemy_fire_projectile.png", new Rectangle(getWorldPosition().getX() + 21, getWorldPosition().getY() + 21, 9 * getgW().TILE_SCALE, 10 * getgW().TILE_SCALE));
