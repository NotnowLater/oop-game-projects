...
public class GrassBossEnemy extends Enemy{
    ...
    private int currentBossShootProjectileId;

    public GrassBossEnemy(GameWindow gW, Vector2 worldPos, String spritePath, int element){
        super(gW, worldPos);
        setElement(element);
        ...
        if (!isDead() && isActive()){
            ...
            if (getCurrentState() == 0){
                ...
                if (isMovingLeft() && (!getgW().getTileManager().isTileBlocking(getHitBox().x - 2, getHitBox().y + getHitBox().height + getCollisionCheckTileOffset()) || getgW().getTileManager().isTileBlocking(getHitBox().x - getCollisionCheckTileOffset(), getHitBox().y))){
                    ...
                    setSpriteWalkId(0);
                    setShootCooldown(1);
                    currentBossShootProjectileId = 0;
                    changeStateTo(3);
    ...
    public void animateSprite(){
        if (getCurrentState() == 0) {
            ...
            if (getSpriteWalkId() >= 0){
                ...
                getVelocity().setY(-20);
                setSpriteWalkId(5);
//                setSpriteWalkId(getSpriteWalkId() + 2);
                changeStateTo(2);
            }
        } else if (getCurrentState() == 3) {
    ...
    public void shootPlayer() {
        ...
        if (getShootDir() == -1) {
            spawnPos = new Vector2(getWorldPosition().getX() + (-14 * getgW().TILE_SCALE), getWorldPosition().getY() + (14 * getgW().TILE_SCALE));
        } else {
            spawnPos = new Vector2(getWorldPosition().getX() + (16 * getgW().TILE_SCALE), getWorldPosition().getY() + (14 * getgW().TILE_SCALE));
        }
        Projectile p = new GrassBossProjectile(getgW(), spawnPos);
        if (getShootDir() == -1){
            p.setFacing(0);
        } else {
            p.setFacing(1);
        }
        p.getVelocity().setX(9 * getShootDir());
        p.getVelocity().setY((currentBossShootProjectileId) * -1);
        getgW().getProjectiles().add(p);
        currentBossShootProjectileId += 2;
        if (currentBossShootProjectileId >= 10){
            changeStateTo(0);
            setShootCooldown(0);
        }
    }
...
//            getgW().getSoundManager().playBGM("select");
            ...
            getgW().getLevelSelect().levelBeat[2] = true;
