...
public class FireBossEnemy extends Enemy{
    ...
    public FireBossEnemy(GameWindow gW, Vector2 worldPos, String spritePath, int element){
        super(gW, worldPos);
        setElement(element);
    ...
    public void process(){
        ...
        if (!isDead() && isActive()){
            ...
            if (getCurrentState() == 0){
                ...
                if (getJumpShootCooldown() >= getMAX_JUMP_SHOOT_COOLDOWN()){
                    if ((getgW().getPlayer().getWorldPosition().getX() <= getWorldPosition().getX() && isMovingLeft()) || (getgW().getPlayer().getWorldPosition().getX() > getWorldPosition().getX() && !isMovingLeft())){
                        changeStateTo(1);
    ...
    public void shootPlayer() {
        ...
        for (int i = -2; i < 3; i++) {
            if (getShootDir() == -1) {
                spawnPos = new Vector2(getWorldPosition().getX() + (2 * getgW().TILE_SCALE), getWorldPosition().getY() + (18 * getgW().TILE_SCALE));
            } else {
                spawnPos = new Vector2(getWorldPosition().getX() + (19 * getgW().TILE_SCALE), getWorldPosition().getY() + (18 * getgW().TILE_SCALE));
            }
            Projectile p = new FireBossProjectile(getgW(), spawnPos);
            p.getVelocity().setX(((24 - Math.abs(i)) * getShootDir()));
            p.getVelocity().setY((6 - Math.abs(i)) * i);
...
//            getgW().getSoundManager().playBGM("select");
            ...
            getgW().getLevelSelect().levelBeat[1] = true;
